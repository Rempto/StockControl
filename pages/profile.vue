            ...
            <v-card class=" pa-8 theme--light" style="border-radius: 20px; margin-top: 5px;">
                <v-row>
                    <v-col>
                        ...
                        <p class="paragraph-2">Atualize as informações do seu perfil e altere sua foto</p>
                    </v-col>
                </v-row>
            </v-card>
        </header>
        ...
        <div>
            <v-card class="  pa-4 my-card" style="border-radius: 20px; margin-top: 5px; ">
                <div class="d-flex justify-center">
                    <div class="v-image v-responsive image theme--light"
                        style="height: 150px; max-width: 150px; background-image: url(&quot;https://portal.keener.io/_nuxt/img/default-account.d1c835b.svg&quot;); background-position: center center;">
                    </div>
                </div>
                ...
                <div class="d-flex justify-end ma-3">
                    <v-btn v-if="changePassword == false" @click="changePassword = true">Alterar senha</v-btn>
                ...
                <div v-if="changePassword" class="ma-0">
                    <v-row class="justify-center ma-2" style="align-items: center;">
                        <v-col :cols="$vuetify.breakpoint.smAndDown ? '12' : '8'" class="pa-0">
                            <v-text-field v-model="oldpassword" type="password" label="Senha atual" required
    ...
    methods: {
        ...
        async change() {
            if (this.newPassword === this.checkPassword) {
                await this.$axios.$put(`user/edit-password?email=${this.user.email}&oldPassword=${this.oldpassword}&newPassword=${this.newPassword}`).then((response) => {
