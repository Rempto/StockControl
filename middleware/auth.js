export default function ({ store, redirect, route, $moment }) {
  ...
  if (route.path === "/" || route.path === "/login/register" || route.path === "/forgetpassword") return;
  ...
  if (token) {
    ...
    if (permission !== 0 && route.path === "/adminRegister") return redirect("/estoque")
    if (permission === 2 && (route.path === "/movimentacao" || route.path === "/produtoregistro")) return redirect("/estoque")
  }
